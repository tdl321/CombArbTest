...
def barrier_frank_wolfe(
    ...
    while epsilon >= config.min_barrier:
        ...
        for t in range(config.max_iterations // 10):
            ...
            logger.debug("[FW] Barrier iteration %d: gap=%.6f, epsilon=%.4f", total_iters, gap, epsilon)
    ...
    logger.info("[FW] Solver converged=%s after %d iterations", converged, total_iters)
...
def find_arbitrage(
    ...
    logger.info("[FW] find_arbitrage called with %d markets, use_barrier=%s", len(market_prices), use_barrier)
    logger.debug("[FW] Markets: %s", list(market_prices.keys()))
    logger.debug("[FW] Input prices: %s", {k: f"{v:.4f}" for k, v in market_prices.items()})
    logger.debug("[FW] Clusters: %d, Total constraints: %d",
                len(relationships.clusters), relationships.computed_relationships)
